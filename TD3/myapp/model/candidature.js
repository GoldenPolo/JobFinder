...
module.exports = {
  ...
  readCandidaturesToAllMyOffers: function (organisation, intituleFilter, callback) {
    db.query('SELECT Candidature.piecesDossier as piecesDossier, Candidature.date as date, Utilisateur.nom as nom, Utilisateur.prenom as prenom, FichePoste.intitule as intitule, FichePoste.responsable as responsable from (Candidature INNER JOIN Offre ON Candidature.offre = Offre.id INNER JOIN Utilisateur ON Candidature.candidat = Utilisateur.id INNER JOIN FichePoste ON Offre.fichePoste = FichePoste.id) WHERE (Offre.organisation = ?) AND (FichePoste.intitule LIKE ?)', [organisation, intituleFilter], function (err, results) {
      if (err) throw err
      callback(results)
    })
  },
